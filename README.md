...
- 🔴 Ecosia (**Currently Excluded by default**) use Cloudflare which now shows a captcha straight away, and doesn't allow you to verify.
- 🔴 Bing (**Currently** Excluded by default) does not like bots. If you show the browser you can solve the captcha and get one page of links but that's all for now.
...
| -es      | --exclude-sources    | Specific sources to exclude searching (`-s google,startpage`). Use `-ls` to display all available sources. If the same source is included with `--sources` and `--exclude-sources`, it will be included.                                                                                                                                                                                                                                                 |
...
| -rwos    | --resubmit-without-subs | After the initial search, search again but exclude all subs found previously to get more links. Ths excludes `Seznam` source because it doesn't recognise that syntax.                                                                                                                                                                                                                                         |
